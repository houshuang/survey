  ...
  def do_op(room, op) do
    ...
    case op do
      {:new_idea, idea_id, user_id, idea} ->
        ...
        userstate = Map.update(userstate, user_id, [idea_id], fn user -> [ idea_id | user ] end)

      {:new_comment, idea_id, user_id, comment} ->
        nick = Survey.User.get_nick(user_id)
        state = Map.update(state, idea_id, %{}, fn idea ->
          %{ idea | comments: [ %{user_id: user_id, user_nick: nick, comment: comment} | idea.comments ] }
        end)

      {:vote, idea_id, user_id} ->
        if not idea_id in Map.get(userstate, user_id, []) do
          state = Map.update(state, idea_id, %{}, fn idea -> %{ idea | score: idea.score + 1 } end)
          userstate = Map.update(userstate, user_id, [idea_id], fn user -> [ idea_id | user ] end)
        end
    end
